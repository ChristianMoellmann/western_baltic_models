...
for y in range(n_years - 1):
    ...
    if recr == 'bev_holt':
        ...
        if y in (np.linspace(1,n_years,num=30)).astype('int'):
            recruits = 82703 # 2017 recruitment
...
plt.plot(2022+np.arange(0,n_years), SSB, color='green', marker='o')
...
plt.figure(figsize=(10, 7))
plt.plot(np.arange(1985,2023), np.array(numbers_df)[:,1]/1e3)

plt.plot(2022+np.arange(0,n_years-2), N[:-2,1]/1e3, color='green', marker='o')
